...
def get_thumbnail_urls(video_id, thumb_format='hq720'):
    """Generate thumbnail URLs for different formats"""
    ...
    thumbnails = {
        ...
        'hq720': f"{base_url}/hq720.jpg"
    }
    
    if thumb_format == 'all':
        return thumbnails
    else:
        return thumbnails.get(thumb_format, thumbnails['hq720'])

def search_youtube_video(query):
    """Search for YouTube video by name and return the first result"""
    ydl_opts = {
        'quiet': True,
        'no_warnings': True,
        'extract_flat': True,
    ...
    try:
        # Prepend ytsearch1: to the query to force YouTube search
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if search_results and 'entries' in search_results and len(search_results['entries']) > 0:
                ...
                if 'id' in entry:
                    ...
                    if len(entry['url']) == 11 and re.match(r'^[A-Za-z0-9_-]+$', entry['url']):
...
def get_video_info(video_url_or_query):
    """Extract video information from YouTube URL or search query"""
    try:
        # Check if it's a URL
        video_id = get_video_id_from_url(video_url_or_query)
        
        if not video_id:
            # It's a search query, find the video first
            video_url = search_youtube_video(video_url_or_query)
            if not video_url:
                return None
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # Extract required information and clean newlines
            ...
            video_info = {
                'title': (info.get('title', 'N/A') or 'N/A').replace('\n', ' ').replace('\r', ' '),
                ...
                'uploader': (info.get('uploader', 'N/A') or 'N/A').replace('\n', ' ').replace('\r', ' '),
            ...
            if video_info['upload_date'] and video_info['upload_date'] != 'N/A':
                try:
                    upload_date = datetime.strptime(video_info['upload_date'], '%Y%m%d')
                    video_info['upload_date_formatted'] = upload_date.strftime('%Y-%m-%d')
                except:
                    video_info['upload_date_formatted'] = video_info['upload_date']
            else:
                video_info['upload_date_formatted'] = 'N/A'
...
def youtube_info():
    ...
    thumb_format = request.args.get('thumb', 'hq720')
    
    if not query:
        return json_response({
            'error': 'Missing query parameter',
            'message': 'Please provide a YouTube URL or video name in the query parameter'
        }, 400)
    ...
    if not video_info:
        return json_response({
            ...
            'message': 'Could not find or extract information from the provided query'
        }, 404)
    
    # Get thumbnail(s) based on format request
    thumbnails = get_thumbnail_urls(video_info['video_id'], thumb_format)
    
    response_data = {
        'title': video_info['title'],
        'duration': video_info['duration'],
        'duration_string': video_info['duration_string'],
        'uploaded': video_info['upload_date_formatted'],
        'views': video_info['view_count'],
        'likes': video_info['like_count'],
        'dislikes': video_info['dislike_count'],
    ...
    # Add thumbnails based on format
    if thumb_format == 'all':
        response_data['thumbnails'] = thumbnails
    else:
        response_data['thumbnail'] = thumbnails
